...
select bidder_id, (100*count(if(bid_status='won',1,null )))/count(bid_status) as percent_wins
 from ipl_bidding_details group by bidder_id order by percent_wins desc;
-- 2.	Display the number of matches conducted at each stadium with stadium name, city from the database.
select * from ipl_stadium;
select * from ipl_match_schedule;
select ip.stadium_id,stadium_name,city,count(match_id) 
from ipl_stadium ip left join ipl_match_schedule im on ip.stadium_id=im.stadium_id 
group by ip.stadium_id,stadium_name,city;
-- 3.	In a given stadium, what is the percentage of wins by a team which has won the toss?
select * from ipl_match;
select * from ipl_match_schedule;
select stadium_id,match_id,team_id1,team_id2,
 100*count(if(toss_winner=match_winner,1,null))/count(match_winner)
 as percct_of_win_by_teams from 
ipl_match_schedule ims left join ipl_match using(match_id) group by stadium_id,match_id,team_id1,team_id2;
-- 4.	Show the total bids along with bid team and team name.
select * from ipl_bidding_Details;
select * from ipl_team;
select * from ipl_bidder_points;
select ipd.bid_team,it.team_id ,sum(no_of_bids),bidder_id,team_name from
 ipl_bidding_Details ipd 
join  ipl_team it on ipd.bid_team=it.team_id 
join ipl_bidder_points using(bidder_id) group by ipd.bid_team,it.team_id ,bidder_id,team_name;
-- 5.	Show the team id who won the match as per the win details.
select * from ipl_match;
select i1.team_id1,i3.team_id2 ,i2.match_winner,i2.win_details
 from ipl_match i1 join ipl_match i2
 on i1.team_id1=i2.match_winner join ipl_match i3 on i3.team_id2=i2.match_winner;
 -- 6.	Display total matches played, total matches won and total matches lost by team along with its team name.
 select * from ipl_team_standings;
 select * from ipl_team;
 select sum(matches_played) as total_matches_played,
 sum(matches_won) as total_matches_won ,sum(matches_lost) 
 as total_matches_lost,team_id,team_name 
 from ipl_team_standings its right join 
 ipl_team it using(team_id) group by team_id,team_name;
 -- 7.	Display the bowlers for Mumbai Indians team.
 select * from ipl_team_players;
 select * from ipl_team;
 select * from ipl_player;
 select player_id, player_name,team_id,team_name,player_role from
 ipl_team join ipl_team_players using(team_id)  
...
select * from ipl_team_players;
 select * from ipl_team;
 select * from ipl_player;
select count(player_role) as  all_rounder_counts, team_id,team_name
from ipl_team_players join ipl_team using(team_id) 
where player_role='All-Rounder' group by team_id,team_name having all_rounder_counts>4 
order by all_rounder_counts desc;
